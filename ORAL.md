...
* **Goal**: Compute A^2 for matrix orders N = 2048 x 2048 and N = 4096 x 4096.
* **Input**: Matrix A initialized with random values from {-1, 0, 1} in row-major layout.
* **Implementations**:
  1. **Ordinary Matrix Multiplication (OMM)** — Non-blocked optimized i-k-j loop order.
...
## Spoken Video Script (Timed for 2.5 to 3.0 Minutes)
...
> "Hello everyone. Today, I am presenting **Problem 1 of our HPC Assignment**, which focuses on parallelizing the computation of A^2 for large matrix orders—2048 x 2048 and 4096 x 4096.
>
...
>
> 1. **Compact Data Types**: Since input entries are strictly {-1, 0, 1}, we stored matrix A using 8-bit integers (`int8_t`). For N=4096, this reduced the memory footprint from 64 MB down to just 16 MB, allowing the entire input to fit comfortably within the L3 cache! For the accumulator and output matrix C, we used 32-bit integers (`int32_t`), ensuring mathematical exactness with zero overflow risk.
>
> 2. **Loop Ordering (i-k-j)**: Instead of traditional i-j-k loops which traverse columns with stride-N cache misses, our OMM uses an i-k-j traversal. This ensures the innermost loop streams through both C[i, *] and A[k, *] sequentially with stride-1 contiguous cache line hits.
>
> 3. **Zero-Sparsity Bypass**: Because A[i, k] is in {-1, 0, 1}, approximately one-third of the elements are zero. If A[i, k] == 0, we bypass the entire inner j-loop, saving billions of compute cycles.
>
> 4. **Tile Parallelization in BMM**: In Block Matrix Multiplication, we collapsed the two outer tile loops (`#pragma omp for collapse(2)`), allowing threads to compute independent B x B tiles of C with zero synchronization locks or false sharing."
...
> * At B=64 for N=4096, the 16-thread runtime dropped to just **1.20 seconds**, outperforming non-blocked OMM by nearly 2x!
...
>
> In summary, combining i-k-j stride-1 locality, 8-bit memory compaction, and OpenMP 2D tile collapsing allowed us to compute 4096 x 4096 matrix squaring in just **1.2 seconds**. Thank you, and I will now hand over to my teammate for Problem 2."
...
3. **Lines 74–107 (`bmm`)**:
   ...
   * Iterates through B x B submatrices (`bs = 4, 8, 16, 32, 64`) keeping active tile data inside L1/L2 caches.
...
* **Q: Why does i-k-j perform so much better than traditional i-j-k?**
  * **Answer**: In i-j-k, accessing column k of matrix B incurs a stride-N jump in memory on every iteration, leading to constant L1/L2 cache misses. In i-k-j, the innermost loop increments j, which reads row A[k, *] and writes row C[i, *] contiguously (stride 1) along cache lines, enabling hardware prefetching and AVX SIMD vectorization.

* **Q: Why did BMM64 outperform non-blocked OMM for N=4096?**
  * **Answer**: At N=4096, the matrix size (16 MB) exceeds L2 cache size. Block size 64 fits 64 x 64 sub-blocks (4 KB) entirely within the CPU's fast L1/L2 caches, eliminating repeated memory trips to main RAM.

* **Q: Why did BMM4 show a super-linear speedup of 20.6x on 16 threads?**
  * **Answer**: On 1 thread, the large footprint of managing millions of tiny 4 x 4 blocks caused cache eviction thrashing. When split across 16 threads, the working set per thread dropped to 1/16th, allowing it to fit into the independent L2 caches of each core, providing super-linear scaling.
